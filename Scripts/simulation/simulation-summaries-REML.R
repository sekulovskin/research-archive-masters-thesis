...
BF <- split(BF.n0.4k_1pred.REML, BF.n0.4k_1pred.REML$rsq)
BF.0  <- BF[["0"]]
BF.1 <- BF[["0.02"]]
BF.2 <- BF[["0.13"]]
BF.3 <- BF[["0.26"]]
...
BF <- split(BF.n3.2k_1pred.REML, BF.n3.2k_1pred.REML$rsq)
BF.0  <- BF[["0"]]
BF.1 <- BF[["0.02"]]
BF.2 <- BF[["0.13"]]
BF.3 <- BF[["0.26"]]
...
BF <- split(BF.n0.4k_2pred.REML, BF.n0.4k_2pred.REML$rsq)
BF.0  <- BF[["0"]]
BF.1 <- BF[["0.02"]]
BF.2 <- BF[["0.13"]]
BF.3 <- BF[["0.26"]]
...
BF <- split(BF.n3.2k_2pred.REML, BF.n3.2k_2pred.REML$rsq)
BF.0  <- BF[["0"]]
BF.1 <- BF[["0.02"]]
BF.2 <- BF[["0.13"]]
BF.3 <- BF[["0.26"]]
...
means_BF.REML <- rbind(means.n0.4k_1pred, means.n0.4k_2pred, means.n3.2k_1pred, means.n3.2k_2pred)

# add indicator for the population

means_BF.REML$setting <- rep(c("N = 400, 1 predictor", "N = 400, 2 predictors", "N = 3200, 1 predictor", "N = 3200, 2 predictors"), each = 4)

rm(means.n0.4k_1pred, means.n0.4k_2pred, means.n3.2k_1pred, means.n3.2k_2pred)

#save

write.csv(means_BF.REML, "simulation-REML.csv")

#Add indicators to which setting to the BF belong to
BF.n0.4k_1pred.REML$setting <- rep(as.factor("N = 400, 1 predictor"),  200) 
BF.n0.4k_2pred.REML$setting <- rep(as.factor("N = 400, 2 predictors"), 200) 
BF.n3.2k_1pred.REML$setting <- rep(as.factor("N = 3200, 1 predictor"), 200) 
BF.n3.2k_2pred.REML$setting <- rep(as.factor("N = 3200, 2 predictors"),200) 



combined_BF.REML <- rbind(BF.n0.4k_1pred.REML, BF.n0.4k_2pred.REML, 
                     BF.n3.2k_1pred.REML, BF.n3.2k_2pred.REML)
combined_BF.REML$BFlog <- log(combined_BF.REML$BF)

write.csv(combined_BF.REML, "combined_BF_REML.csv")
