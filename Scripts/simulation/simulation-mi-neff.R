...
Tau.B[1:3,1:3] ~ dwish(Sigma.B, 3)  # inverse of covariance matrix following a Wishard dist.
  ...
  Tau.B[1,1] <- pow(sigma.alpha, 2)  #construct the cov matrix
  Tau.B[2,2] <- pow(sigma.beta_1, -1/2)
  Tau.B[3,3] <- pow(sigma.beta_2, -1/2)
  Tau.B[1,2] <- rho_1*sigma.alpha*sigma.beta_1 #covariance between slope of b1 and intercept
  Tau.B[2,1] <- Sigma.B[1,2]
  Tau.B[1,3] <- rho_2*sigma.alpha*sigma.beta_2 #covariance between slope of b2 and intercept
  Tau.B[3,1] <- Sigma.B[1,3]
  Tau.B[2,3] <- rho_3*sigma.beta_1*sigma.beta_2 #covariance between slope of b2 and slope of b1
  Tau.B[3,2] <- Sigma.B[2,3]
  rho_1 ~ dunif(-1, 1)  #cor is between -1 and 1
