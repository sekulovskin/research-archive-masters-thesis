...
tau ~ dgamma (0.001, 0.001)  #resiudal variance
invSigma.B[1:3,1:3] ~ dwish(Tau.B, 3)  # inverse of covariance matrix following a Wishard dist.
  tau.alpha ~ dgamma (0.001, 0.001)  #ntercept variance
  tau.beta_1 ~ dgamma (0.001, 0.001)    #slope variance
  tau.beta_2 ~ dgamma (0.001, 0.001)    #slope variance
  Tau.B[1,1] <- pow(tau.alpha, 2)  #construct the cov matrix
  Tau.B[2,2] <- pow(tau.beta_1, -1/2)
  Tau.B[3,3] <- pow(tau.beta_2, -1/2)
  Tau.B[1,2] <- rho_1*tau.alpha*tau.beta_1 #covariance between slope of b1 and intercept
  Tau.B[2,1] <- Tau.B[1,2]
  Tau.B[1,3] <- rho_2*tau.alpha*tau.beta_2 #covariance between slope of b2 and intercept
  Tau.B[3,1] <- Tau.B[1,3]
  Tau.B[2,3] <- rho_3*tau.beta_1*tau.beta_2 #covariance between slope of b2 and slope of b1
  Tau.B[3,2] <- Tau.B[2,3]
  rho_1 ~ dunif(-1, 1)  #cor is between -1 and 1
