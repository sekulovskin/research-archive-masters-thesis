...
#'
#' @param x fitted \code{lmer} model
#' @param hypotheses specified hypotheses for \code{bain} (see:https://cran.r-project.org/web/packages/bain/vignettes/Introduction_to_bain.html)
#' @param fraction size (multiplicative factor) of the minimal value for J. Defaults to 1 (the minimal fraction)
#' @param standardize when set to \code{TRUE}, applies overall data standardization.  Defaults to \code{FALSE}
#' @param N  sample size to be used. Options: 1) N = "level_1" - uses the level 1 observations;
# 2) N = "level_2" - uses the level 2 observations; 3) N = "ICC_effective" (default option)- uses the effective sample size
...
#' @param seed setting the value for the random seed (users are advised to run the analysis with two random seeds, in order to ensure stability of the results)
#' @param jref If \code{Jref} is set to TRUE then J_ref is used (derived by having the BF = 19, when the effect size
# for the fixed effects is zero in the data). In this case the supplied value for the argument fraction is ignored. Defaults to
#' \code{FALSE}
#'
#' @return returns the standard \code{bain} output with the calculated fit, complexity BF_.u and PMPs for each hypothesis.
#' 
#'
#' @examples 
#' 
#' library(R2MLwiN)
#' data("tutorial") # load the data 
#' 
...
#' 
#' 
#' results <- bain_2lmer(x, hypotheses, 
#'                standardize = FALSE, N = "level_1")
#'      
#' 
#' 
#' 
bain_2lmer <- function(x, hypotheses, fraction = 1, standardize = FALSE, 
                       N = "ICC_effective", seed = 123, jref = FALSE){
  options(warn = -1)
    ...
    if(N == "level_1"){
      ...
      if(standardize == FALSE){ 
        ...
        results <- bain::bain(fixed_estimates_s, hypotheses, n = N, Sigma = cov_s,
                              group_parameters = 0, 
                              joint_parameters = length(fixed_estimates_s), 
                              fraction = fraction)
      }
    }
