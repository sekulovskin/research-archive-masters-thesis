...
mi.Neff.2pred.nG45 <- function(data){
  ...
  samples <- rbind(chain1, chain2) #combine both chains
  #Get the fixed effects
  fixed_alphas <- samples[, 136]   
  fixed_betas_1 <- samples[, 137]
  fixed_betas_2 <- samples[, 138]
  fixed_alphas <- as.data.frame(fixed_alphas)
  fixed_betas_1 <- as.data.frame(fixed_betas_1)
  fixed_betas_2 <- as.data.frame(fixed_betas_2)
  ...
  n_gr <- sapply(split, nrow)
  #exclude the fixed effects
  samples <- samples[, -c(136,137,138)]
  
  alphas <- samples [, 1:45] #extract the intercepts
  betas_1 <- samples [, 46:90] #extract the slopes
  betas_2 <- samples [, 91:135] #extract the slopes
...
#
# nG = 20, 1 pred
mi.Neff.1pred.nG20.0 <- function(data){
  ...
  vCOV <- list()
  for(i in seq_along(imputed)){
    estimates[[i]] <- coef(lm(imputed[[i]][,6] ~ 1))
    vCOV [[i]] <- vcov(lm(imputed[[i]][,6] ~ 1))
  }
  #Extract each type of coefficient
...
# nG = 20, 2 pred
mi.Neff.2pred.nG20.0 <- function(data){
...
# nG = 80 1 pred
mi.Neff.1pred.nG80.0 <- function(data){
...
# nG = 80 2 pred
mi.Neff.2pred.nG80.0 <- function(data){
